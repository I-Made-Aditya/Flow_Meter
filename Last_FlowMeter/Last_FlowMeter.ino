...
void loop() {
  ...
  if(peak == (tP+500.0) || peak == (tP-500.0) ){
    zPeak = 1;
  }
  // Serial.println(volt);
  Serial.print(peak);
  ...
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("F : " + String(peak) + "");
  lcd.setCursor(0,1);
  lcd.print("D : " + String(debit/1000,6) + " m^3");  
...
double calculateFlowRate(double receivedFrequency, double transmissionFrequency, double relativeAngle) {
  ...
  //   flowRate = 0.0;
  // }
  if (receivedFrequency == 0 || receivedFrequency == transmissionFrequency || zPeak == 1 ) {
    flowRate = 0.0;
    zPeak = 0;
  }
